...
def parse_featured_text(text: str) -> list:
    ...
    i = 0
    while i < len(lines):
        # Look for "Pin" followed by "Compare" — that's the start of a card
        if lines[i] == "Pin" and i + 1 < len(lines) and lines[i + 1] == "Compare":
            card_lines = []
            j = i + 2  # Skip "Pin" and "Compare"
            
            # Collect lines until next "Pin" or "Explore More Internships" or "Dashboard"
            while j < len(lines):
...
def parse_card_lines(card_lines: list) -> dict:
    ...
    # Find sector/field and state from remaining lines
    fields = []
    state = ""
    for l in clean[2:]:
        # Indian states are typically ALL CAPS
    ...
    if not state:
        # Check if state is embedded in a field line with /
        for l in clean[2:]:
...
def infer_skills(title: str, sector: str = "") -> list:
    text = f"{title} {sector}".lower()
    skill_map = {
        "computer": "computer skills", "software": "software development",
        "engineer": "engineering", "mechanic": "mechanical skills",
        "sales": "sales", "marketing": "marketing", "merchandis": "merchandising",
        "finance": "financial analysis", "account": "accounting",
        "data": "data entry", "admin": "administration",
        "customer": "customer service", "hr": "human resources",
        "design": "product design", "research": "research & analysis",
        "mainten": "maintenance", "plumb": "plumbing",
        "electr": "electrical work", "weld": "welding",
        "manufactur": "manufacturing", "logistics": "logistics",
        "relation": "public relations", "trainee": "on-the-job training",
        "intern": "communication",
    }
    found = [skill for kw, skill in skill_map.items() if kw in text]
...
def scrape_pm_portal() -> list:
    ...
    # Scrape with different geolocation to get diverse results
    geolocations = [
    ...
    with sync_playwright() as p:
        ...
        for geo in geolocations:
            ...
            # Scroll to Featured Internships
            ...
            # Click carousel right arrow to get more
